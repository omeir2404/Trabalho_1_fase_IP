...
public class IpurdleTest {
	...
	private static void testIsValidClue () {
		...
		int clue = 111;
		int size = 3;
		if (!Ipurdle_Nexus.validClue (clue,size)) {
	...
	private static void testMinClue () {
		...
		int obtainedClue = Ipurdle_Nexus.minClue (size);
		...
		size = 5;
		expectedClue = 11111;
		obtainedClue = Ipurdle_Nexus.minClue (size);
		if (obtainedClue != expectedClue) {
			System.out.printf (">>> expected: %d obtained: %d %n", expectedClue, obtainedClue);
			error = true;
		}
	...
	private static void testIsMaxClue () {
		...
		int size = 3;
		int clue = 333;
		if (!Ipurdle_Nexus.isMaxClue (clue, size)) {
	...
	private static void testNextClue () {
		...
		int obtainedClue = Ipurdle_Nexus.nextClue (clue, size);
		...
		obtainedClue = Ipurdle_Nexus.nextClue (clue, size);
		if (obtainedClue != expectedNextClue) {
			System.out.printf (">>> failed on next clue for %d %n", clue);
			System.out.printf (">>> expected: %d obtained: %d %n", expectedNextClue, obtainedClue);
			error = true;
	...
	private static void testClueForGuessAndWord () {
		...
		int obtainedClue = Ipurdle_Nexus.clueForGuessAndWord (guess,word);
		if (obtainedClue != expectedClue) {
		...
		word = "REBUT";
		expectedClue = 23211;
		obtainedClue = Ipurdle_Nexus.clueForGuessAndWord (guess, word);
		if (obtainedClue != expectedClue) {
			System.out.printf (">>> failed on clue for %s and %s %n", guess, word);
			System.out.printf (">>> expected: %d obtained: %d %n", expectedClue, obtainedClue);
			error = true;
	...
	private static void testHowManyWordsWithClue () {
		...
		int clue = 11211;
		int expectedNum = 4;
		int obtainedNum = Ipurdle_Nexus.howManyWordsWithClue (dictionary, clue, guess);
		...
		clue = 23211;
		expectedNum = 3;
		obtainedNum = Ipurdle_Nexus.howManyWordsWithClue (dictionary, clue, guess);
		if (obtainedNum != expectedNum) {
			System.out.printf (">>> failed on  %d and %s %n", clue, guess);
			System.out.printf (">>> expected: %d obtained: %d %n", expectedNum, obtainedNum);
			error = true;
	...
	private static void testBetterClueForGuess() {
		...
		int obtainedNum = Ipurdle_Nexus.betterClueForGuess (dictionary, guess);
		if (obtainedNum != expectedNum) {
		...
		expectedNum = 11211;
		obtainedNum = Ipurdle_Nexus.betterClueForGuess (dictionary, guess);
		if (obtainedNum != expectedNum) {
	...
	private static void testPlayGuess() {
		...
		int obtainedClue = Ipurdle_Nexus.playGuess (dictionary, guess);
		if (obtainedClue != expectedClue) {
		...
		expectedClue = 23211;
		obtainedClue = Ipurdle_Nexus.playGuess (dictionary, guess);
		if (obtainedClue != expectedClue) {
